...
(function () {
    ...
    const maxWeeksToCheck =4; //check upto 8 weeks(4 months)
  ...
  //     alert("Script Is Running");
   // });
    ...
    function getWeekRange(){
        ...
        const weekRangeElement = document.querySelector('.centre.bold');

        if(weekRangeElement) {
            const weekRangeText = weekRangeElement.textContent.trim();
            //Extract the start and end dates
            const [startDate, endDate] = weekRangeText.split(' - ');
            return {startDate, endDate};
        }
        return null; //return null if week range cannot be found
    }
    ...
    function getAvailableDates() {
        ...
        const dateCells = document.querySelectorAll('.day.slotsavailable');

        dateCells.forEach(cell => {
          const day = cell.querySelector('abbr')?.textcontent.trim();
    ...
    function formatExactDate(day, date, month, year) {
    ...
    function step6(){
        ...
        if (availableDates.length > 0) {
            ...
            const formattedDates = availableDates.map(({ day, date }) => {
               return formattedDates(day, date, month, year);
            });
    ...
    function step7(){
        ...
        if(nextWeekLink && !dateFound){
         ...
         setTimeout(() => {
           ...
           console.log("No more weeks to check or reached the limit");
           showToast("Finished checking availability");
